  ...
  return {
    ...
    hooks: {
      "astro:config:setup"({ config, updateConfig, logger }) {
        const srcDirName = path
          .relative(
            url.fileURLToPath(config.root),
            ...
            plugins: [
              {
                ...
                resolveId(source) {
                  if (source === "astro:import") {
                    return this.resolve(
                      "astro-dynamic-import/runtime/virtual-module.ts"
                    );
                  }
                ...
                load(id) {
                  if (id === "astro-dynamic-import:internal") {
                    return (
                      ...
                      `export const lookupMap = import.meta.glob('/${srcDirName}/widgets/*.astro', { query: { ${PROPAGATED_ASSET_FLAG}: true } })\n`
                    );
                  }
                },
              },
